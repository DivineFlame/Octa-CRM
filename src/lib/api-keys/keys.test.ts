...
describe('hashApiKey', () => {
  it('is deterministic', () => {
    expect(hashApiKey('octacrm_live_abc')).toBe(hashApiKey('octacrm_live_abc'));
  });

  it('differs for different inputs', () => {
    expect(hashApiKey('octacrm_live_abc')).not.toBe(hashApiKey('octacrm_live_abd'));
  });
});
...
describe('timingSafeHexEqual', () => {
  it('is true for identical digests', () => {
    const h = hashApiKey('octacrm_live_xyz');
