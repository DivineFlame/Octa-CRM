...
The public API lets you drive your OctaCRM instance from your own
...
```
Authorization: Bearer octacrm_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** OctaCRM
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer octacrm_live_xxx"
```
...
Rather than polling, register an endpoint and OctaCRM will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; OctaCRM keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer octacrm_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/OctaCRM", "events": ["message.received"] }'
...
Headers: `X-OctaCRM-Event`, `X-OctaCRM-Webhook-Id`, and
`X-OctaCRM-Signature`.

### Verifying the signature

`X-OctaCRM-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages OctaCRM stores (inbox + API sends), not broadcast-only
...
[#245](https://github.com/DivineFlame/Octa-CRM/issues/245). Future ideas
